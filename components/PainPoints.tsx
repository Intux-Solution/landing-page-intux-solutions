      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8">
        {cards.map((card, idx) => (
          <FadeIn key={idx} delay={0.2 + idx * 0.1} className="h-full">
            <div className="bg-bgLight dark:bg-darkCard rounded-[2rem] p-8 md:p-10 h-full flex flex-col items-start transition-all duration-300 border border-transparent hover:border-white/10 hover:bg-gradient-to-br hover:from-darkCard hover:to-white/5 hover:scale-[1.02] group">
              <div className={`w-12 h-12 rounded-full ${card.iconBg} flex items-center justify-center mb-6 transition-transform group-hover:scale-110 duration-300`}>
                {card.icon}
              </div>
